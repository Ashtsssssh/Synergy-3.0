...
function clearChunkQueue(ctx) {
	const chunkIds = [];
	for (const row of ctx.db.chunkQueue.iter()) chunkIds.push(row.chunkId);
	for (const chunkId of chunkIds) ctx.db.chunkQueue.chunkId.delete(chunkId);
}
function seedChunkQueue(ctx, config) {
	clearChunkQueue(ctx);
...
const request_work = spacetimedb.reducer((ctx) => {
	markNodeAlive(ctx);
	for (const inFlight of ctx.db.chunkQueue.chunk_queue_by_assigned_node.filter(ctx.sender)) if (inFlight.status === "processing") return;
