...
//#region D:/Synergy/Synergy-3.0/node_modules/headers-polyfill/lib/index.mjs
...
//#endregion
//#region D:/Synergy/Synergy-3.0/node_modules/spacetimedb/dist/server/index.mjs
...
//#endregion
//#region D:/Synergy/Synergy-3.0/spacetimedb/src/index.ts
...
const DEFAULT_PIN_TOTAL_CANDIDATES = 1e6;
const DEFAULT_PIN_CHUNK_SIZE = 1e4;
...
function incrementDonatedCount(ctx) {
...
function upsertTask(ctx, taskId, taskKey, displayName, isActive, requestHelp) {
...
function getTaskOrThrow(ctx, taskId) {
	const task = ctx.db.task.taskId.find(taskId);
	if (!task) throw new SenderError("Task does not exist.");
	return task;
}
function clearMandelbrotChunks(ctx) {
	const chunkIds = [];
	for (const row of ctx.db.mandelbrotChunkQueue.iter()) chunkIds.push(row.chunkId);
	for (const chunkId of chunkIds) ctx.db.mandelbrotChunkQueue.chunkId.delete(chunkId);
}
function clearPinChunks(ctx) {
	const chunkIds = [];
	for (const row of ctx.db.pinChunkQueue.iter()) chunkIds.push(row.chunkId);
	for (const chunkId of chunkIds) ctx.db.pinChunkQueue.chunkId.delete(chunkId);
}
function seedMandelbrotChunks(ctx, config) {
	...
	for (let tileY = 0; tileY < config.rows; tileY += 1) for (let tileX = 0; tileX < config.cols; tileX += 1) {
...
function seedPinChunks(ctx, config) {
	clearPinChunks(ctx);
	for (let rangeStart = 0; rangeStart < config.totalCandidates; rangeStart += config.chunkSize) {
		const rangeEnd = Math.min(config.totalCandidates - 1, rangeStart + config.chunkSize - 1);
...
function upsertGridConfig(ctx, config) {
...
function upsertPinConfig(ctx, config) {
...
function assignMandelbrotChunk(ctx) {
	let inFlightCount = 0;
	for (const inFlight of ctx.db.mandelbrotChunkQueue.mandelbrot_chunk_queue_by_assigned_node.filter(ctx.sender)) if (inFlight.status === "processing") {
		inFlightCount += 1;
		if (inFlightCount >= MAX_INFLIGHT_CHUNKS_PER_NODE) return;
	}
...
function assignPinChunk(ctx) {
	if (ctx.db.pinCrackConfig.id.find(1)?.pinFound) return;
	let inFlightCount = 0;
	for (const inFlight of ctx.db.pinChunkQueue.pin_chunk_queue_by_assigned_node.filter(ctx.sender)) if (inFlight.status === "processing") {
		inFlightCount += 1;
		if (inFlightCount >= MAX_INFLIGHT_CHUNKS_PER_NODE) return;
	}
...
function resetMandelbrotTaskToPending(ctx) {
	for (const chunk of ctx.db.mandelbrotChunkQueue.iter()) ctx.db.mandelbrotChunkQueue.chunkId.update({
		...chunk,
		status: "pending",
		assignedNode: void 0,
		pixelData: void 0,
...
function resetPinTaskToPending(ctx) {
	for (const chunk of ctx.db.pinChunkQueue.iter()) ctx.db.pinChunkQueue.chunkId.update({
		...chunk,
		status: "pending",
		assignedNode: void 0,
		foundPin: void 0,
	...
	if (pinConfig) ctx.db.pinCrackConfig.id.update({
		...pinConfig,
		pinFound: void 0,
		foundByNode: void 0,
		foundAtMicros: void 0,
...
const init = spacetimedb.init((ctx) => {
	upsertTask(ctx, MANDELBROT_TASK_ID, "mandelbrot", "Mandelbrot Rendering", true, true);
	upsertTask(ctx, PIN_TASK_ID, "pin_guess", "PIN Guessing", true, true);
	const defaultGrid = {
	...
	if (!hasMandelbrotChunks) seedMandelbrotChunks(ctx, defaultGrid);
	const defaultPinConfig = {
		targetHash: "4ed8dfd7183bd310f609b89ed2c2e20edcaf0d2aadeb8b3668ab9bb52428874b",
	...
	if (!hasPinChunks) seedPinChunks(ctx, defaultPinConfig);
});
const onConnect = spacetimedb.clientConnected((_ctx) => {});
const onDisconnect = spacetimedb.clientDisconnected((_ctx) => {});
const heartbeat = spacetimedb.reducer((ctx) => {
	markNodeAlive(ctx);
});
const set_task_help = spacetimedb.reducer({
	taskId: t.u32(),
	requestHelp: t.bool()
...
const set_task_active = spacetimedb.reducer({
	taskId: t.u32(),
	isActive: t.bool()
}, (ctx, { taskId, isActive }) => {
	const task = getTaskOrThrow(ctx, taskId);
...
const reset_task = spacetimedb.reducer({ taskId: t.u32() }, (ctx, { taskId }) => {
	const task = getTaskOrThrow(ctx, taskId);
...
const request_work = spacetimedb.reducer({ taskId: t.u32() }, (ctx, { taskId }) => {
	markNodeAlive(ctx);
	const task = getTaskOrThrow(ctx, taskId);
	if (!task.isActive || !task.requestHelp) return;
	if (task.taskKey === "mandelbrot") {
...
const submit_result = spacetimedb.reducer({
	taskId: t.u32(),
	chunkId: t.u64(),
	...
	const task = getTaskOrThrow(ctx, taskId);
	if (task.taskKey === "mandelbrot") {
		if (!resultData) throw new SenderError("Mandelbrot resultData is required.");
		const chunk = ctx.db.mandelbrotChunkQueue.chunkId.find(chunkId);
		if (!chunk) throw new SenderError("Chunk does not exist.");
		if (chunk.status !== "processing") throw new SenderError("Chunk is not in processing state.");
		if (!chunk.assignedNode || chunk.assignedNode.toHexString() !== ctx.sender.toHexString()) throw new SenderError("Chunk is assigned to another node.");
		ctx.db.mandelbrotChunkQueue.chunkId.update({
			...chunk,
			status: "completed",
	...
	if (task.taskKey === "pin_guess") {
		...
		if (!chunk) throw new SenderError("PIN chunk does not exist.");
		if (chunk.status !== "processing") throw new SenderError("PIN chunk is not in processing state.");
		if (!chunk.assignedNode || chunk.assignedNode.toHexString() !== ctx.sender.toHexString()) throw new SenderError("PIN chunk is assigned to another node.");
		...
		const pinConfig = ctx.db.pinCrackConfig.id.find(1);
		if (pinConfig && foundPin && !pinConfig.pinFound) ctx.db.pinCrackConfig.id.update({
			...pinConfig,
			pinFound: foundPin,
...
const reset_grid = spacetimedb.reducer({
	cols: t.u32(),
	rows: t.u32(),
	maxIterations: t.u32(),
	reMin: t.f64(),
	...
	if (cols < 2 || rows < 2) throw new SenderError("cols and rows must be >= 2");
	if (cols > 200 || rows > 200) throw new SenderError("cols and rows must be <= 200");
	if (maxIterations < 100 || maxIterations > 1e4) throw new SenderError("maxIterations must be between 100 and 10000");
	if (reMax <= reMin || imMax <= imMin) throw new SenderError("Invalid complex plane bounds.");
	const nextConfig = {
		cols,
		rows,
		maxIterations,
...
const reset_pin_crack = spacetimedb.reducer({ targetHash: t.string() }, (ctx, { targetHash }) => {
	const normalizedHash = targetHash.trim().toLowerCase();
	if (!/^[0-9a-f]{64}$/.test(normalizedHash)) throw new SenderError("targetHash must be a 64-char hex SHA-256 value.");
	const nextConfig = {
...
//#endregion
export { spacetimedb as default, heartbeat, init, onConnect, onDisconnect, request_work, reset_grid, reset_pin_crack, reset_task, set_task_active, set_task_help, submit_result };
//# debugId=d364ff6d-0c6a-434c-aef7-3150014a8c57
//# sourceMappingURL=data:application/json;charset=utf-8;base64,