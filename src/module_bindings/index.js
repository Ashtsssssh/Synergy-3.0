...
var DbConnectionImpl = class {
  ...
  #sendMessage(message) {
    ...
    if (this.ws && this.isActive) {
      ...
      stdbLogger(
        "trace",
        () => `Queuing message to server: ${stringify2(message)}`
      );
...
var chunk_queue_table_default = t.row({
  ...
  tileY: t.u32().name("tile_y"),
  minRe: t.f64().name("min_re"),
  maxRe: t.f64().name("max_re"),
  minIm: t.f64().name("min_im"),
  maxIm: t.f64().name("max_im"),
  width: t.u32(),
  height: t.u32(),
...
var tablesSchema = schema({
  ...
  gridConfig: table({
    name: "grid_config",
    indexes: [
      { accessor: "id", name: "grid_config_id_idx_btree", algorithm: "btree", columns: [
        "id"
      ] }
    ],
    constraints: [
  ...
  nodeStatus: table({
    name: "node_status",
    indexes: [
      { accessor: "node_status_by_last_seen_micros", name: "node_status_last_seen_micros_idx_btree", algorithm: "btree", columns: [
        "lastSeenMicros"
      ] },
      { accessor: "nodeId", name: "node_status_node_id_idx_btree", algorithm: "btree", columns: [
        "nodeId"
      ] }
    ],
    constraints: [
