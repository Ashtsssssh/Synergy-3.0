...
const inProgressEl = document.getElementById('chunks-in-progress');
...
DbConnection.builder()
	...
	.onConnect(conn => {
		...
		conn.db.mandelbrotChunkQueue.onInsert((_ctx, row) => {
		...
		conn.db.mandelbrotChunkQueue.onUpdate((_ctx, _oldRow, row) => {
			if (row.status === 'completed') {
				paintChunk(row);
			}
			updateMetrics(conn);
	...
	.onConnectError((_ctx, err) => {
		console.error('Mandelbrot live dashboard connection error:', err);
	})
	.build();
